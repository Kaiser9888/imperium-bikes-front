import type { VideoItem, VideoPage } from "./types";

/** Mesma origem de API do projeto original. */
...
function demoPage(page: number, isShort: boolean): VideoPage {
    const content: VideoItem[] = Array.from({ length: PAGE_SIZE }).map((_, i) => {
        ...
        return {
            ...
            thumbnailUrl: "",
            formattedDuration: isShort ? `0:${20 + (n % 39)}` : `${mins}:${String(10 + (n % 49)).padStart(2, "0")}`,
...
export async function fetchVideoPage(page: number, isShort = false): Promise<VideoPage> {
    try {
        const res = await fetch(`${API_URL}/api/videos?page=${page}&size=${PAGE_SIZE}&isShort=${isShort}`, { signal: AbortSignal.timeout(4000) });
