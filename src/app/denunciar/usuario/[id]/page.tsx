...
const motivos = [
    'Perfil falso ou identidade falsa',
    'Assedio ou comportamento abusivo',
    'Spam ou propaganda indesejada',
    'Tentativa de golpe ou fraude',
    'Conteudo ofensivo no perfil',
    'Varias contas do mesmo usuario',
    'Menor de idade',
    'Outro motivo'
]

export default function DenunciarUsuarioPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true)
        try {
            // await api.post('/api/reports', { userId: Number(id), reason: motivo, description: descricao })
        ...
        <div style={{ minHeight: '100vh', backgroundColor: '#f5f5f5', paddingBottom: '70px' }}>
            ...
            <div style={{ backgroundColor: '#fff', borderBottom: '1px solid #e5e5e5', padding: '10px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <button onClick={() => window.history.back()} style={{ display: 'flex', padding: '4px', background: 'none', border: 'none', cursor: 'pointer' }}>
                        <ArrowLeft size={20} color="#1a1a1a" />
                    </button>
                    <h1 style={{ fontSize: '17px', fontWeight: '700', color: '#1a1a1a' }}>Denunciar usuario</h1>
                </div>
            </div>
            ...
            <div style={{ padding: '16px' }}>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    ...
                    <div>
                        <label style={labelStyle}><Flag size={14} />Descricao *</label>
                        <textarea value={descricao} onChange={(e) => setDescricao(e.target.value)}
                                  placeholder="Descreva detalhadamente o ocorrido..." rows={5} maxLength={500}
                        ...
                        <p style={{ fontSize: '11px', color: '#aaa', marginTop: '4px', textAlign: 'right' }}>{descricao.length}/500</p>
                    </div>
                    ...
                    <button type="submit" disabled={loading || !motivo || !descricao.trim()} style={{
                        ...
                        cursor: (loading || !motivo || !descricao.trim()) ? 'not-allowed' : 'pointer',
