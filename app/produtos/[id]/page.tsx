...
export default function ProdutoPage() {
  ...
  // Estados de UI (não vêm do banco)
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#f5f3ee] px-6 text-center">
        <div>
          ...
          <h1 className="mt-4 font-serif text-4xl">Anúncio não encontrado</h1>
  ...
  // ---------- NORMALIZAÇÃO ----------
  // Como o ProductResponse pode variar, normalizamos aqui.
  const p = produto as ProductResponse & {
    title?: string
    name?: string
    brand?: string
    year?: number | string
  ...
  const titulo = p.title ?? p.name ?? "Produto"
  const marca = p.brand ?? ""
  const ano = p.year ?? ""
  const descricao =
    p.description ?? "Este anúncio ainda não possui uma descrição."
  const preco = p.price

  const imagens = p.photos?.length
    ? p.photos
    : p.images?.length
      ? p.images
      : p.imageUrl
  ...
  const vendedorNome =
    typeof p.seller === "string" ? p.seller : p.seller?.name ?? "Vendedor"
  ...
  const sellerSince = p.seller_since ?? p.sellerSince ?? ""
  const rating = p.rating ?? "—"
  const reviews = p.reviews ?? 0
  const activeListings = p.active_listings ?? p.active ?? 0
  const specs: [string, string][] = p.specs ?? []
  ...
  const calculateShipping = () => {
    ...
    setShipping("R$ 89,90 · entrega em até 5 dias úteis")
  }
    ...
    <main className="min-h-screen bg-[#f5f3ee] text-[#1d282b]">
      <header className="border-b border-[#dedbd2] bg-[#f8f7f3]/95">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
          ...
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em]">
            ...
            <span className="hidden sm:block">Imperium</span>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-7xl px-5 py-8 lg:px-8 lg:py-12">
        ...
        >
          ...
          <span>/</span>
          <span className="text-[#1d282b]">{marca}</span>
          <span>/</span>
          <span className="text-[#1d282b]">{titulo}</span>
        </nav>
              ...
              {imagens.length > 1 && (
                <>
                  <button
                    onClick={() =>
                      setActiveImage(
                        (activeImage + imagens.length - 1) % imagens.length,
                      )
                    }
                  ...
                  <button
                    onClick={() =>
                      setActiveImage((activeImage + 1) % imagens.length)
                    }
          ...
          <div className="flex flex-col">
            <div className="flex items-start justify-between gap-4">
              ...
              >
                <Heart
                  className={`size-5 ${
                    favorite ? "fill-[#a33c36] text-[#a33c36]" : "text-[#68737a]"
                  }`}
                />
              </button>
            </div>
            ...
            <div className="mt-7 flex items-end justify-between border-b border-[#dedbd2] pb-7">
              <div>
                <p className="mb-1 text-xs text-[#68737a]">Preço anunciado</p>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-[1fr_auto]">
              ...
              >
                ...
                {bought ? "Interesse registrado" : "Comprar agora"}
              </button>
              <button className="flex items-center justify-center gap-2 border border-[#1d282b] px-5 py-4 text-sm font-bold hover:bg-white">
            ...
            <div className="mt-5 border border-[#dedbd2] bg-[#faf9f5] p-5">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-[#1d282b] px-4 text-xs font-bold uppercase tracking-[0.08em] text-white hover:bg-[#a33c36]"
                >
                  Calcular
                </button>
              </div>
              {shipping && (
                <p className="mt-3 flex items-center gap-2 text-xs font-semibold text-[#386148]">
                  <Check className="size-4" /> {shipping}
                </p>
              )}
            </div>
            ...
            <div className="mt-6 border border-[#dedbd2] bg-[#faf9f5] p-5">
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="grid size-12 place-items-center rounded-full bg-[#d9d8d1] text-sm font-bold">
                    {iniciais || <User className="size-5" />}
                  </div>
                  <div>
                    <p className="font-semibold">{vendedorNome}</p>
                    {sellerSince && (
                      <p className="mt-1 text-xs text-[#68737a]">
                        Vendedor desde {sellerSince}
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="mt-4 flex items-center gap-5 border-t border-[#dedbd2] pt-4 text-xs text-[#68737a]">
                ...
                <span>{activeListings} anúncios ativos</span>
              </div>
            </div>
          </div>
        </section>
          ...
          <div>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              <div className="flex gap-3 border-t border-[#dedbd2] pt-4">
                ...
                <div>
                  <p className="text-sm font-semibold">Envio protegido</p>
              ...
              <div className="flex gap-3 border-t border-[#dedbd2] pt-4">
                ...
                <div>
                  <p className="text-sm font-semibold">Anúncio verificado</p>
                  <p className="mt-1 text-xs leading-5 text-[#68737a]">
            ...
            <aside className="border-l border-[#dedbd2] pl-0 lg:pl-8">
              ...
              <dl className="flex flex-col gap-4 text-sm">
                {specs.map(([label, value]) => (
                  ...
                  >
                    <dt className="text-[#68737a]">{label}</dt>
