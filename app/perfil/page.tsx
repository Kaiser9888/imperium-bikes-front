...
export default function PerfilPage() {
    ...
    const toggleSeguir = () => {
        setJaSegue(!jaSegue)
        setSeguidores((prev) => prev + (jaSegue ? -1 : 1))
    }
                ...
                {aba === "torneios" && (
                    <div className="space-y-3">
                        {torneiosFake.map((torneio) => (
                            <div key={torneio.id} className="flex items-center gap-4 rounded-xl border border-border bg-card p-4 hover:border-primary/30 transition-colors">
                ...
                <div className="fixed inset-0 z-50 flex flex-col bg-black">
                    <div className="flex items-center justify-between px-4 py-3">
                        ...
                        <div className="flex items-center gap-1">
                            <button onClick={() => excluirFoto(fotoSelecionada)} className="flex size-9 items-center justify-center rounded-full bg-white/10 text-white hover:bg-red-500/50 transition-colors">
                                <Trash2 className="size-4" />
                            </button>
                            <button onClick={fecharFoto} className="flex size-9 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20 transition-colors">
                                <X className="size-4" />
                            </button>
                        </div>
                    </div>
