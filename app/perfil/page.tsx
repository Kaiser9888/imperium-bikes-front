...
import { Camera, Package, Trophy, Grid3X3, ShoppingBag, Medal, Settings, MapPin, Link2, X, Flame, Send, Trash2, ChevronRight, Plus, Pencil } from "lucide-react"
...
export default function PerfilPage() {
    ...
    const [seguidores] = useState(1247)
    const [seguindo] = useState(389)
    ...
    const salvarEditRapido = () => {
        // Futuro: PUT /api/users/{id}
        ...
        <div className="min-h-screen bg-background">
            ...
            <main className="mx-auto max-w-2xl px-4 py-8">
                ...
                <div className="mt-8 grid grid-cols-3 gap-2">
                    {[{ valor: fotos.length, label: "Fotos" },{ valor: 0, label: "Produtos" },{ valor: torneiosFake.length, label: "Torneios" }].map((stat) => (
                            ...
                            {fotos.length === 0 ? (
                                ...
                                <>
                                    ...
                                    <div className="grid grid-cols-3 gap-1.5">
                                        {fotos.map((foto, i) => (
                                            <button key={i} onClick={() => modoExclusao ? excluirFoto(i) : abrirFoto(i)} onContextMenu={(e) => { e.preventDefault(); setModoExclusao(true) }} className="aspect-square bg-secondary rounded-lg overflow-hidden hover:opacity-80 transition-opacity group relative">
                                                ...
                                                {modoExclusao ? <div className="absolute inset-0 bg-red-500/40 flex items-center justify-center"><Trash2 className="size-6 text-white" /></div>
                                                    : <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-end justify-start p-2 opacity-0 group-hover:opacity-100"><Flame className="size-3 fill-white mr-1" />{contagemCurtidas[i]}</div>}
                                            </button>
                                        ))}
                                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
                    ...
                    <div className="relative bg-background rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md p-6 shadow-2xl">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="font-heading text-base font-bold text-foreground">Editar perfil</h3>
                        ...
                        <div className="space-y-4">
                            <div><label className="text-[10px] text-muted-foreground uppercase tracking-wider">Bio</label><textarea value={editBio} onChange={(e) => setEditBio(e.target.value)} rows={2} className="w-full rounded-xl border border-border bg-card px-4 py-3 text-sm mt-1 outline-none focus:border-primary/30 resize-none" /></div>
                        ...
                        <div className="flex gap-2 mt-6">
                            ...
                            <button onClick={salvarEditRapido} className="flex-1 rounded-xl bg-primary py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90">Salvar</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {fotoSelecionada !== null && (
                <div className="fixed inset-0 z-50 flex flex-col bg-black">
                    <div className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-3"><img src={user?.imageUrl||"/placeholder.svg"} alt="Avatar" className="size-8 rounded-full object-cover" /><span className="text-sm font-medium text-white">{user?.fullName||"Usuário"}</span></div>
                    ...
                    <div className="flex-1 flex items-center justify-center px-2"><img src={fotos[fotoSelecionada]} alt="Foto" className="max-h-full max-w-full rounded-lg object-contain"/></div>
                    <div className="px-4 py-3 flex items-center gap-4">
                        <button onClick={()=>toggleCurtida(fotoSelecionada)} className="transition-transform active:scale-125"><Flame className={`size-6 ${curtidas[fotoSelecionada]?"fill-orange-500 text-orange-500":"text-white"}`}/></button>
                    ...
                    <div className="bg-background rounded-t-2xl max-h-64 overflow-y-auto">
                        <div className="p-4 space-y-3">{comentarios.length===0&&<p className="text-sm text-muted-foreground text-center py-4">Nenhum comentário ainda.</p>}{comentarios.map((c,i)=><div key={i}><span className="text-sm font-semibold text-foreground">{c.autor}</span> <span className="text-sm text-muted-foreground">{c.texto}</span></div>)}</div>
                        <div className="flex items-center gap-2 border-t border-border/50 px-4 py-2.5">
                            <input type="text" value={comentario} onChange={(e)=>setComentario(e.target.value)} placeholder="Adicionar comentário..." className="flex-1 bg-transparent text-[13px] outline-none placeholder:text-muted-foreground/50" onKeyDown={(e)=>{if(e.key==="Enter"&&comentario.trim())enviarComentario()}}/>
