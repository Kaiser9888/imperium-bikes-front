...
export default function VideosLayout({
  ...
  // ============================================================
  // MEMENTO: SEM TOP BAR (tem a própria no MementoPage)
  // ============================================================
  ...
  // ============================================================
  // WATCH PAGE: SEM TOP BAR (só o conteúdo)
  // ============================================================
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      {!isSearchPage && (
        ...
        >
          <div className="flex h-14 items-center gap-2 px-3 sm:px-6 lg:px-8">
            <Link href="/videos" className="flex flex-shrink-0 items-center gap-2" aria-label="Página inicial de vídeos">
              <img src="/logo2.png" alt="Imperium" className="h-7 w-auto sm:h-8" />
              <span
                className="text-sm tracking-wide sm:text-2xl"
                style={{
            ...
            <div className="ml-auto flex flex-shrink-0 items-center gap-1">
              <Link href="/videos/buscar" className="rounded-full p-2 text-foreground transition-colors hover:bg-muted hover:text-primary active:scale-95" aria-label="Buscar vídeos" title="Buscar vídeos">
                <Search className="h-5 w-5" aria-hidden="true" />
              </Link>

              <button type="button" className="rounded-md p-2 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" aria-label="Notificações">
              ...
              {isSignedIn ? (
                ...
                <SignInButton mode="modal">
                  <button type="button" className="flex h-8 w-8 items-center justify-center rounded-full bg-muted text-muted-foreground transition-colors hover:bg-primary/10 hover:text-primary" aria-label="Entrar">
      ...
      {/* CONTEÚDO */}
      <main className={`flex-1 ${!isSearchPage ? "pt-14" : ""} pb-16`}>
        {children}
      </main>
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-around px-2">
          {navItems.map((item) => {
            ...
            if (item.highlight) {
              return (
                <Link key={item.label} href={item.href} className="flex flex-col items-center gap-0.5 py-2" aria-label={item.label} aria-current={isActive ? "page" : undefined}>
            ...
            return (
              <Link key={item.label} href={href} className={`flex min-w-[64px] flex-col items-center gap-0.5 py-2 transition-colors ${
