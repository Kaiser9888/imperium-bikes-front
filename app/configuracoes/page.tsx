...
interface Patrocinador { nome: string; logo: string }
        ...
        <div className="min-h-screen bg-background">
            ...
            <main className="mx-auto max-w-2xl px-4 py-6 space-y-8">
                ...
                <section>
                    ...
                    <div className="space-y-3">
                        <label className="flex items-center justify-between rounded-xl border border-border bg-card p-4 cursor-pointer">
                            ...
                            <input type="checkbox" checked={mostrarSeguidores} onChange={(e) => setMostrarSeguidores(e.target.checked)} className="accent-primary" />
                        </label>
                        <label className="flex items-center justify-between rounded-xl border border-border bg-card p-4 cursor-pointer">
                            <span className="text-sm text-foreground">Mostrar seguindo publicamente</span>
                            <input type="checkbox" checked={mostrarSeguindo} onChange={(e) => setMostrarSeguindo(e.target.checked)} className="accent-primary" />
                        </label>
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div className="space-y-3">
                        <div>
                            <label className="text-[10px] text-muted-foreground uppercase tracking-wider">Preço mensal para patrocinadores</label>
                            <div className="flex items-center gap-2 mt-1">
                                <span className="text-muted-foreground">R$</span>
                                <input type="number" value={precoPatrocinio} onChange={(e) => setPrecoPatrocinio(e.target.value)} placeholder="49,90" className="w-32 rounded-xl border border-border bg-card px-3 py-2.5 text-sm outline-none focus:border-primary/30" />
                ...
                <section>
                    ...
                    {mostrarFormTorneio && (
                        <div className="rounded-xl border border-border bg-card p-4 mb-3 space-y-3">
                            <input type="text" value={novoTorneio.nome} onChange={(e) => setNovoTorneio({ ...novoTorneio, nome: e.target.value })} placeholder="Nome do torneio" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none" />
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-[10px] text-muted-foreground uppercase">Posição</label>
                                    <select value={novoTorneio.posicao} onChange={(e) => setNovoTorneio({ ...novoTorneio, posicao: Number(e.target.value) })} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm mt-1 outline-none">
                                ...
                                <div>
                                    ...
                                    <input type="text" value={novoTorneio.data} onChange={(e) => setNovoTorneio({ ...novoTorneio, data: e.target.value })} placeholder="Mar 2026" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm mt-1 outline-none" />
                                </div>
                            </div>
                            <button onClick={adicionarTorneio} className="w-full rounded-lg bg-primary py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">Salvar torneio</button>
                        </div>
                    )}
                    ...
                    <div className="space-y-2">
                        {torneios.map((t) => (
                            <div key={t.id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3">
                                ...
                                <button onClick={() => removerTorneio(t.id)} className="text-muted-foreground hover:text-red-500"><Trash2 className="size-4" /></button>
                            </div>
                        ))}
                    </div>
                </section>

                <button onClick={salvar} className="w-full rounded-xl bg-primary py-3 text-sm font-semibold text-primary-foreground hover:bg-primary/90 flex items-center justify-center gap-2">
